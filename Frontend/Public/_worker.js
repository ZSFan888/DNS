export async function onRequest(context) {
  ...
  try {
    ...
    if (url.pathname === '/api/root-domains') { const mod = await import('../../src/api-root-domains.js'); return request.method === 'GET' ? mod.onRequestGet({ request, env, session }) : mod.onRequestPost({ request, env, session }); }
    ...
    if (url.pathname === '/api/subdomains') { const mod = await import('../../src/api-subdomains.js'); return request.method === 'GET' ? mod.onRequestGet({ request, env, session }) : mod.onRequestPost({ request, env, session }); }
    ...
    if (url.pathname === '/api/dns-records') { const mod = await import('../../src/api-dns-records.js'); return request.method === 'GET' ? mod.onRequestGet({ request, env, session }) : mod.onRequestPost({ request, env, session }); }
    ...
    return new Response(JSON.stringify({ ok: false, error: 'internal_error', message: String(err && err.message || err) }), { status: 500, headers: { 'content-type': 'application/json' } });
  }
}
